...
final class JdbcWorkflowInstanceRepository implements WorkflowInstanceRepository {
    ...
    @Override public Optional<WorkflowSnapshot> findById(WorkflowInstanceId id) {
        return findOne("select " + columns() + " from workflow_instance where id = ?", id.toString());
    }
    ...
    @Override public Optional<WorkflowSnapshot> findByCorrelationId(String correlationId) {
        ...
        List<WorkflowSnapshot> matches=queryMany("select "+columns()+" from workflow_instance where correlation_id=? order by updated_at,id limit 2",correlationId);
    ...
    @Override public Optional<WorkflowSnapshot> findActiveById(WorkflowInstanceId id) {
        return findOne("select " + columns() + " from workflow_instance where id = ? and status in ('RUNNING','WAITING','FAILED')", id.toString());
    }
    ...
    @Override public List<WorkflowSnapshot> findActiveByCorrelation(String workflowKey,String correlationId,int limit) {
        ...
        return queryMany("select "+columns()+" from workflow_instance where workflow_key=? and correlation_id=? and status in ('RUNNING','WAITING','FAILED') order by updated_at,id limit ?",workflowKey,correlationId,limit);
    }
    ...
    @Override public List<WorkflowSnapshot> findActiveByBusinessKey(String workflowKey,String businessKey,int limit) {
        ...
        return queryMany("select "+columns()+" from workflow_instance where workflow_key=? and business_key=? and status in ('RUNNING','WAITING','FAILED') order by updated_at,id limit ?",workflowKey,businessKey,limit);
    }

    @Override public List<WorkflowSnapshot> findActiveAfter(ActiveWorkflowCursor cursor,int limit) {
        if(limit<1)throw new IllegalArgumentException("limit must be positive");
        if(cursor==null)return queryMany("select "+columns()+" from workflow_instance where status in ('RUNNING','WAITING','FAILED') order by updated_at,id limit ?",limit);
        return queryMany("select "+columns()+" from workflow_instance where status in ('RUNNING','WAITING','FAILED') and (updated_at>? or (updated_at=? and id>?)) order by updated_at,id limit ?",cursor.updatedAt().toString(),cursor.updatedAt().toString(),cursor.instanceId().toString(),limit);
    }
    ...
    @Override public Optional<WorkflowSnapshot> findByBusinessKey(String workflowKey, String businessKey) {
        ...
        String sql = "select " + columns() + " from workflow_instance where workflow_key = ? and business_key = ? order by created_at desc limit 1";
        try (Connection connection = connectionFactory.open(); PreparedStatement statement = connection.prepareStatement(sql)) {
    ...
    @Override public List<WorkflowSnapshot> findActive(int limit) {
        if (limit < 1) throw new IllegalArgumentException("limit must be positive");
        String sql = "select " + columns() + " from workflow_instance where status in ('RUNNING','WAITING','FAILED') order by updated_at, id limit ?";
        try (Connection connection = connectionFactory.open(); PreparedStatement statement = connection.prepareStatement(sql)) {
    ...
    @Override public List<WorkflowSnapshot> findAll(int limit, int offset) {
        requirePage(limit,offset);return queryMany("select "+columns()+" from workflow_instance order by created_at,id limit ? offset ?",limit,offset);
    }

    @Override public List<WorkflowSnapshot> findByStatus(WorkflowStatus status,int limit) {
        Objects.requireNonNull(status,"status");if(limit<1)throw new IllegalArgumentException("limit must be positive");
        return queryMany("select "+columns()+" from workflow_instance where status=? order by updated_at,id limit ?",status.name(),limit);
    }

    @Override public List<WorkflowSnapshot> findStuck(Instant updatedBefore,int limit) {
        Objects.requireNonNull(updatedBefore,"updatedBefore");if(limit<1)throw new IllegalArgumentException("limit must be positive");
        String sql="select "+columns()+" from workflow_instance where status in ('RUNNING','WAITING') and updated_at<=? order by updated_at,id limit ?";
    ...
    private List<WorkflowSnapshot> queryMany(String sql,Object... parameters){try(Connection c=connectionFactory.open();PreparedStatement s=c.prepareStatement(sql)){for(int i=0;i<parameters.length;i++){Object value=parameters[i];if(value instanceof Integer n)s.setInt(i+1,n);else s.setString(i+1,String.valueOf(value));}try(ResultSet rows=s.executeQuery()){ArrayList<WorkflowSnapshot> result=new ArrayList<>();while(rows.next())result.add(map(rows));return List.copyOf(result);}}catch(SQLException x){throw new WorkflowInfrastructureException("Failed workflow instance query",x);}}
    private static void requirePage(int limit,int offset){if(limit<1)throw new IllegalArgumentException("limit must be positive");if(offset<0)throw new IllegalArgumentException("offset must not be negative");}
    private static void requireRoute(String workflowKey,String value,int limit){if(workflowKey==null||workflowKey.isBlank())throw new IllegalArgumentException("workflowKey is required");if(value==null||value.isBlank())throw new IllegalArgumentException("routing value is required");if(limit<1)throw new IllegalArgumentException("limit must be positive");}
    ...
    private static String columns() {
        return "id, workflow_key, workflow_version, workflow_revision, business_key, correlation_id, current_state, status, variables, lock_version, created_at, updated_at";
    }
