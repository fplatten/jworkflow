...
public final class JdbcInboxApplication implements AutoCloseable {
    ...
    public InboxInsertResult accept(InboxMessage message){message=capture(Objects.requireNonNull(message));
        ...
        return persistence.jdbcTransactions().inImmediateTransaction(()->persistence.inbox().insertIfAbsent(safe));
    }
    ...
    public int pollOnce(){Instant now=clock.instant();
        ...
        List<InboxMessage> claimed=persistence.jdbcTransactions().inImmediateTransaction(()->persistence.inbox().claimEligible(now,workerId,now.plus(lease),batchSize));
    ...
    private void process(InboxMessage message){try{
        persistence.jdbcTransactions().inImmediateTransaction(() -> processing.process(message,workerId));
    }catch(RuntimeException failure){finalizeFailure(message,failure);
    }}
    private void finalizeFailure(InboxMessage message,RuntimeException failure){int attempt=message.attemptCount()+1;
        ...
        boolean exhausted=retryPolicy.exhausted(attempt);
        ...
        if(exhausted)persistence.inbox().markDeadLetter(message.messageId(),workerId,error,clock.instant());
            else persistence.inbox().scheduleRetry(message.messageId(),workerId,next,error);
        });
            if(exhausted)lifecycleObserver.observe(new WorkflowLifecycleEvent(WorkflowLifecycleEventType.INBOX_DEAD_LETTERED,clock.instant(),null,null,null,null,null,message.correlationId(),message.causationId(),null,Map.of("sourceSystem",message.sourceSystem(),"failureCategory","inbox_processing_failed")));
        }
