...
public final class JdbcQueryContractTest {
 ...
 private static void queriesCompleteReadModelsWithoutMutation()throws Exception{
  ...
  try(var engine=ContractBackend.engine(f.url).initialize(true).timerPolling(false).build()){check(engine.queries().list(10,0).size()==3,"durable engine must expose the persistence-backed query API");}
 }
 ...
 private static Fixture fixture()throws Exception{String url=ContractBackend.url(Files.createTempFile("jworkflow-query-",".sqlite").toAbsolutePath());return new Fixture(url,ContractBackend.persistence(url,null,null,null,null,true,Map.of()));}private static int count(Fixture f,String table)throws Exception{try(var c=ContractBackend.open(f.url);var s=c.createStatement();var r=s.executeQuery("select count(*) from "+table)){return r.next()?r.getInt(1):0;}}private static void check(boolean value,String message){if(!value)throw new AssertionError(message);}private record Fixture(String url,JdbcWorkflowPersistence persistence){}
