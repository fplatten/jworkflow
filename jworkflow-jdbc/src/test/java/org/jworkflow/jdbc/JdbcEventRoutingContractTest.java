...
public final class JdbcEventRoutingContractTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        tenantRoutesAreRejectedWithoutWrites();
    }
    ...
    private static void tenantRoutesAreRejectedWithoutWrites() throws Exception {
        Fixture fixture=fixture("tenant");
        try(JdbcWorkflowEngine engine=engine(fixture,wait("tenant-flow"))){
            WorkflowInstanceId id=start(engine,"tenant-flow","employee-t","corr-t");
            WorkflowSnapshot before=engine.snapshot(id);
            int events=engine.queries().timeline(id).entries().size();
            WorkflowEvent incoming=event("tax.completed",id,"corr-t","employee-t",Map.of(),Map.of());
            for(WorkflowEventRoute route:List.of(
                    new WorkflowEventRoute(id,null,null,null,"tenant-a",incoming.eventName(),WorkflowRoutingMode.SINGLE),
                    new WorkflowEventRoute(null,"tenant-flow","employee-t",null,"tenant-a",incoming.eventName(),WorkflowRoutingMode.SINGLE))){
                try{engine.route(incoming,route);throw new AssertionError("tenant route must be rejected");}
                catch(InvalidWorkflowRouteException expected){ }
            }
            check(before.equals(engine.snapshot(id)),"tenant route mutated workflow");
            check(events==engine.queries().timeline(id).entries().size(),"tenant route appended events");
        }
    }
    ...
    private static void concurrentDeliveryCannotOverwrite() throws Exception {
        ...
        try(JdbcWorkflowEngine engine=engine(fixture,wait("concurrent"))){
            ...
            Future<Object>a=pool.submit(work),b=pool.submit(work);gate.countDown();Object left,right;try{left=a.get(15,java.util.concurrent.TimeUnit.SECONDS);right=b.get(15,java.util.concurrent.TimeUnit.SECONDS);}finally{pool.shutdownNow();}
    ...
    private static void payloadAndSchemaReachStepHandler()throws Exception{
        ...
        WorkflowEngineBuilder builder=ContractBackend.engine(fixture.url()).initialize().timerPolling(false).definition(definition).stepHandler("tax.receive",context->{received.set(context.signal().message());return StepResult.success();});
    ...
    private static JdbcWorkflowEngine engine(Fixture fixture,WorkflowDefinition...definitions)throws Exception{WorkflowEngineBuilder builder=ContractBackend.engine(fixture.url()).initialize().timerPolling(false).eventRoutingMaximumCandidates(10).lazyDefinitionValidation(true);for(WorkflowDefinition definition:definitions)builder.definition(definition);return(JdbcWorkflowEngine)builder.build();}
    private static Fixture fixture(String name)throws Exception{Path file=Files.createTempFile("jworkflow-routing-"+name+"-",".sqlite");return new Fixture(ContractBackend.url(file.toAbsolutePath()));}
