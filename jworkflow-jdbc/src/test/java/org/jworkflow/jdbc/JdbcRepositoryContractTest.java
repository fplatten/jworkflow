...
public final class JdbcRepositoryContractTest {
    ...
    private static void definitionRoundTripAndConflicts() throws Exception {
        ...
        try(Connection c=ContractBackend.open(f.url);Statement s=c.createStatement()){s.executeUpdate("update workflow_definition set canonical_json='{}' where workflow_key='orders'");}
        expect(PersistenceConstraintException.class,()->f.persistence.definitions().save(definition));
        try(Connection c=ContractBackend.open(f.url);Statement s=c.createStatement()){s.executeUpdate("update workflow_definition set canonical_json='"+escaped(new JdbcDefinitionCodec().write(definition))+"',checksum='bad' where workflow_key='orders'");}
    ...
    private static void timerStateMachineAndClaims() throws Exception {
        ...
        timers.markFailed(first.timerId(),"worker-a",claimed.get(0).claimToken(),"temporary",now.plusSeconds(10));check(timers.dueTimers(now).isEmpty(),"retry must honor next-attempt time");
        WorkflowTimer retry=f.persistence.jdbcTransactions().inImmediateTransaction(()->timers.claimDue(now.plusSeconds(11),"worker-a",now.plusSeconds(40),1)).get(0);timers.markFired(retry.timerId(),"worker-a",retry.claimToken(),now.plusSeconds(12));
    ...
    private static void inboxDeduplicationAndStateMachine() throws Exception {
        ...
        UUID id=claimed.get(0).messageId();f.persistence.inbox().appendAttempt(new InboxAttempt(null,id,1,InboxMessageStatus.RETRY_SCHEDULED,"temporary","retry",now));f.persistence.inbox().scheduleRetry(id,"inbox-a",claimed.get(0).claimToken(),now.plusSeconds(2),"retry");
        InboxMessage retried=f.persistence.jdbcTransactions().inImmediateTransaction(()->f.persistence.inbox().claimEligible(now.plusSeconds(3),"inbox-b",now.plusSeconds(20),1)).get(0);f.persistence.inbox().markProcessed(retried.messageId(),"inbox-b",retried.claimToken(),now.plusSeconds(4));check(f.persistence.inbox().findById(id).orElseThrow().status()==InboxMessageStatus.PROCESSED,"inbox must reach processed state");check(f.persistence.inbox().findAttempts(id).size()==1,"inbox attempts must append");
        InboxMessage dead=inbox("external-2",now);f.persistence.inbox().insertIfAbsent(dead);UUID deadId=f.persistence.jdbcTransactions().inImmediateTransaction(()->f.persistence.inbox().claimEligible(now,"inbox-c",now.plusSeconds(10),1)).get(0).messageId();f.persistence.inbox().markDeadLetter(deadId,"inbox-c",f.persistence.inbox().findById(deadId).orElseThrow().claimToken(),"exhausted",now);check(f.persistence.inbox().findById(deadId).orElseThrow().status()==InboxMessageStatus.DEAD_LETTER,"inbox must dead-letter");
    ...
    private static void outboxIdempotencyAndStateMachine() throws Exception {
        ...
        f.persistence.outbox().appendAttempt(new OutboxAttempt(null,claimed.messageId(),1,OutboxMessageStatus.RETRY_SCHEDULED,"broker","down",now));f.persistence.outbox().scheduleRetry(claimed.messageId(),"publisher-a",claimed.claimToken(),now.plusSeconds(2),"down");OutboxMessage retry=f.persistence.jdbcTransactions().inImmediateTransaction(()->f.persistence.outbox().claimEligible(now.plusSeconds(3),"publisher-b",now.plusSeconds(20),1)).get(0);f.persistence.outbox().markPublished(retry.messageId(),"publisher-b",retry.claimToken(),now.plusSeconds(4));check(f.persistence.outbox().findById(retry.messageId()).orElseThrow().status()==OutboxMessageStatus.PUBLISHED,"outbox must reach published state");check(f.persistence.outbox().findAttempts(retry.messageId()).size()==1,"outbox attempts must append");
        OutboxMessage dead=outbox("publish-2",now);f.persistence.outbox().enqueue(dead);OutboxMessage deadClaim=f.persistence.jdbcTransactions().inImmediateTransaction(()->f.persistence.outbox().claimEligible(now,"publisher-c",now.plusSeconds(1),1)).get(0);check(f.persistence.outbox().releaseExpiredClaims(now.plusSeconds(2))==1,"expired outbox lease must release");deadClaim=f.persistence.jdbcTransactions().inImmediateTransaction(()->f.persistence.outbox().claimEligible(now.plusSeconds(2),"publisher-d",now.plusSeconds(20),1)).get(0);f.persistence.outbox().markDeadLetter(deadClaim.messageId(),"publisher-d",deadClaim.claimToken(),"exhausted",now.plusSeconds(3));check(f.persistence.outbox().findById(deadClaim.messageId()).orElseThrow().status()==OutboxMessageStatus.DEAD_LETTER,"outbox must dead-letter");
    }
    ...
    private static Fixture fixture(String name)throws Exception{Path db=Files.createTempFile("jworkflow-repository-"+name+"-",".sqlite");String url=ContractBackend.url(db.toAbsolutePath());JdbcWorkflowPersistence first=ContractBackend.persistence(url,null,null,null,null,true,Map.of("sqlite.busy-timeout-ms","1000"));JdbcWorkflowPersistence second=ContractBackend.persistence(url,null,null,null,null,false,Map.of("sqlite.busy-timeout-ms","1000"));return new Fixture(url,first,second);}
