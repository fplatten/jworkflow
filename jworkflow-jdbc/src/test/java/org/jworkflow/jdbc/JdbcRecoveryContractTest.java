...
public final class JdbcRecoveryContractTest {
    ...
    private static void dueTimerExecutesExactlyOnceAfterRestart() throws Exception {
        ...
        JdbcWorkflowPersistence ports=ContractBackend.persistence(f.url,null,null,null,null,false,Map.of());WorkflowTimer timer=ports.timers().findByWorkflowInstance(id).get(0);
    ...
    private static void expiredTimerClaimIsRecoveredButValidLeaseIsNotStolen() throws Exception {
        ...
        f.clock.advance(Duration.ofSeconds(6));JdbcWorkflowPersistence ports=ContractBackend.persistence(f.url,null,null,null,null,false,Map.of());
    ...
    private static void missingRevisionAndChecksumMismatchFailClearly() throws Exception {
        ...
        try(var c=ContractBackend.open(missing.url);var s=c.createStatement()){s.executeUpdate("update workflow_instance set workflow_revision='missing-revision'");}
        ...
        try(var c=ContractBackend.open(checksum.url);var s=c.createStatement()){s.executeUpdate("update workflow_definition set checksum='corrupt'");}
    ...
    private static void competingRecoveringEnginesDoNotExecuteOneTimerTwice() throws Exception {
        ...
        try(JdbcWorkflowEngine one=engine(f,null,counter);JdbcWorkflowEngine two=engine(f,null,counter)){
            ExecutorService pool=Executors.newFixedThreadPool(2);Future<Integer>a=pool.submit(one::pollTimersOnce),b=pool.submit(two::pollTimersOnce);int claims=a.get(15,java.util.concurrent.TimeUnit.SECONDS)+b.get(15,java.util.concurrent.TimeUnit.SECONDS);pool.shutdownNow();check(claims==1,"competing engines must produce one timer owner");check(observations.get()==1,"claimed timer must execute once");}
    }
    ...
    private static Fixture fixture(String name)throws Exception{Path file=Files.createTempFile("jworkflow-recovery-"+name+"-",".sqlite");return new Fixture(ContractBackend.url(file.toAbsolutePath()),new MutableClock(Instant.parse("2026-01-01T00:00:00Z")));}
