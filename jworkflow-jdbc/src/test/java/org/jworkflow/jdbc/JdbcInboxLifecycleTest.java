...
public final class JdbcInboxLifecycleTest {
    ...
    private static void duplicateAndConcurrentAcceptanceDispatchOnce()throws Exception{
        Fixture f=fixture("duplicate",5);try(JdbcWorkflowEngine engine=engine(f,definition())){try(JdbcInboxApplication app=engine.inbox(translator())){
            ...
            InboxMessage concurrent=message("external-2","erp","order-2");ExecutorService pool=Executors.newFixedThreadPool(2);try{Future<InboxInsertResult>a=pool.submit(()->app.accept(concurrent)),b=pool.submit(()->app.accept(concurrent));InboxInsertResult ar=a.get(),br=b.get();check(ar.inserted()!=br.inserted(),"concurrent duplicate must insert once");}finally{pool.shutdownNow();}app.pollOnce();check(engine.context().getWorkflows().size()==2,"concurrent duplicate must have one workflow effect");
        }}}
